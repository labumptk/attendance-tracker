...
export async function createAttendanceList(listName: string, listPassword: string, startAt: Date, durationMinutes = defaultDurationMinutes, hostPassword?: string) {
  const cookieStore = await cookies()
  ...
  const parsedStartAt = startAtSchema.safeParse(startAt)
  ...
  if (!parsedStartAt.success || Number.isNaN(parsedStartAt.data.getTime())) return { error: 'Start time is invalid.' }
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    if (existing.length === 0) {
      ...
      const savedStartAt = parsedStartAt.data
      const expiresAt = new Date(savedStartAt.getTime() + parsedDuration.data * 60 * 1000)
      await db.insert(attendanceLists).values({ id, listName: parsedName.data, creatorPassword, listPassword: parsedPassword.data, createdAt, startAt: savedStartAt, expiresAt })
      return { id, startAt: savedStartAt, expiresAt }
    }
  }
